...
class DistanceVector(Node):
    ...
    def process_BF(self):
        ...
        #             for i in value:
        #                 if i in msg[name]:
        #                     if i == msg['sender']: # do not need to process anything
        #                         pass
        #                     else:
        ...
        #                                 break
        #                         self.dist_vector[i] = new_distance
        updated_status = False #send message or not
        
        for msg in self.messages:            
            for i in msg['dist']: #msg has origin and vector with nodeweight
                if i in self.dist_vector and i != self.name:
                    old_weight = int(self.get_outgoing_neighbor_weight(msg['sender']))
                    msg_weight = int(msg['dist'][i])
                    new_weight = int(old_weight + msg_weight)
                    if (old_weight <= -99 or msg_weight <= -99 or new_weight <= -99) and self.dist_vector[i] != -99: # no update if it is already -99
                        updated_status = True
                        self.dist_vector[i] = -99
                    elif self.dist_vector[i] != -99 and new_weight < self.dist_vector[i]:
                        updated_status = True
                        self.dist_vector[i] = new_weight
                    ...
                    for out in self.outgoing_links:
                        if i == out:
                            ...
                            new_weight = int(self.get_outgoing_neighbor_weight(msg['sender'])) + int(msg['dist'][i]) #get to node + node itself
                    self.dist_vector[i] = new_weight        
        # Empty queue
        self.messages = []
